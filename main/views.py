...
from .forms import CommentAcademician, CreateNewList, CommentLecture, CommentLectureAcademician
...
def academician(response, name):
    ...
    if Comment.objects.filter(academician = academician.id).__len__() > 0:
...
def lecture(response, name):
    ...
    form = CommentLecture()
    form.fields["academician"].queryset = Academician.objects.filter(lecture__name__startswith = name)
    ...
    return render(response, "main/lecture.html", {"lecture" : lecture, "form" : form, "comment" : comment, "user" : user, "academicians" : academicians, "name" : name})
...
def lecture_academician(response, lecture, academician):
    user = response.user
    lecture = Lecture.objects.get(name = lecture)
    academician = Academician.objects.get(name = academician)
    form = CommentLectureAcademician()
    comment = Comment.objects.all()

    if response.method == "POST":
        form = CommentLectureAcademician(response.POST,  response.FILES)
        if form.is_valid():
            form.instance.user = response.user
            form.instance.lecture = lecture
            form.instance.academician = academician
            form.save()

    return render(response, "main/lecture_academician.html", {"lecture" : lecture, "comment" : comment, "user" : user, "academician" : academician, "form" : form})

def leaderboard(response):
    user = response.user
    comments = Comment.objects.all()
    academicians = Academician.objects.all()
    board = comments.values('academician').annotate(Avg('rating'), Avg('grade')).order_by('-rating')
    
    return render(response, "main/leaderboard.html",{"user" : user, "academicians" : academicians, "board": board})